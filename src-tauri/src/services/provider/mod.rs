...
mod tests {
    ...
    fn update_active_aggregate_provider_without_takeover_is_allowed() {
        with_test_home(|state, _| {
            ...
            state
                .db
                .set_current_provider("claude", "aggregate")
                .unwrap();
            crate::settings::set_current_provider(&AppType::Claude, Some("aggregate")).unwrap();
...
impl ProviderService {
    ...
    pub fn delete(state: &AppState, app_type: AppType, id: &str) -> Result<(), AppError> {
        ...
        if matches!(
            app_type,
            AppType::Claude | AppType::ClaudeScience | AppType::ClaudeDesktop | AppType::Codex
        ) {
    ...
    fn sync_common_config_snippet_from_live(
        ...
        // cc-switch 写 live 时注入的 permissions.deny "WebSearch" 哨兵不得进
        // 共享片段（否则会被合并进所有勾选通用配置的供应商）。剥离需要 provider
        // 做判定并区分用户手写的 deny，extract_claude_common_config 的签名拿不到
        // provider，因此在调用点先做剥离。
        let mut live_config = live_config.clone();
        if matches!(app_type, AppType::Claude) {
    ...
    /// 校验聚合供应商路由：仅 Claude / Claude Science / Claude Desktop / Codex
    /// 应用支持；约定 Claude 系（含 Science / Desktop）只校验四档（tiers）路由、
    /// Codex 只校验 custom（自由键）路由，互相忽略。
    ...
    fn validate_aggregate_routes(
        ...
        if !matches!(
            ...
            return Err(AppError::localized(
                ...
                "聚合供应商仅支持 Claude / Claude Science / Claude Desktop / Codex 应用",
                "Aggregate providers are only supported for the Claude, Claude Science, Claude Desktop and Codex apps",
            ));
        }
        ...
        match app_type {
            AppType::Claude | AppType::ClaudeScience | AppType::ClaudeDesktop => {
                for (tier, route) in [
