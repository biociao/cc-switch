...
**Stats**: 33 commits | 114 files changed | +13,894 insertions | -1,687 deletions

### Fixed

- **Codex Third-Party Providers Forced Into ChatGPT OAuth**: Codex 0.144+ treats `requires_openai_auth = true` as an explicit ChatGPT-login requirement. The flag leaked into third-party live configs from three directions: the bearer-token injection used by the takeover / preserved-official-login path, the verbatim live write used by a plain third-party switch, and every new-provider template (custom preset, partner presets, Grok Build presets, NewAPI universal form, deep-link import). All three are now normalized or cleaned — injecting a provider-scoped `experimental_bearer_token` flips the flag to `false`, a direct switch normalizes legacy stored configs before writing the live config, and no generator emits the flag for API-key providers anymore. Official ChatGPT OAuth channels keep the flag deliberately and are untouched. Redo of the stalled upstream PR #5222 per farion1231/cc-switch#4393, extended beyond both open upstream PRs (#5469, #5828) with the direct-switch normalization and template cleanup.
- **Codex Backfill Leaked the Live-Only Auth Flag Into Stored Configs**: after the live projection started carrying `requires_openai_auth = false`, the backfill that lifts a live bearer token back into `auth.OPENAI_API_KEY` would also persist the synthetic flag into the stored provider config. The flag is now restored from the stored template on backfill (dropped when the template has none), keeping live-projection state out of the database. Port of upstream PR #5828.
- **Claude Science Model Picker Empty, Session Model "(unavailable)"**: the Science daemon filters `/v1/models` entries whose `display_name` is all-lowercase kebab-case (treating them as internal ids). cc-switch emitted upstream model ids and route ids verbatim, so the entire list was discarded and the picker rendered empty. The Science models response now humanizes machine-shaped labels (`claude-opus-5` → `Claude Opus 5`, `deepseek-v4-pro-260425` → `Deepseek V4 Pro 260425`; already-human labels like `Kimi K2.7` or `GPT-5.4` pass through unchanged) while keeping entry `id`/`name` on the route id so the daemon still matches saved session models. The Claude Desktop models response is unchanged.
- **Claude Science Model List Without a Desktop Route Table** (previously unreleased): `/claude-science/v1/models` fell over for providers that only carry Claude Code-style env model config; it now falls back to projecting the env tiers (`ANTHROPIC_DEFAULT_*_MODEL` / `ANTHROPIC_MODEL`) onto the Claude-safe route ids, and the Science provider form gained the Form/JSON view with Base URL backfill.
- **Kimi for Coding Presets Reported Wrong Context Windows** (previously unreleased): the Kimi presets are pinned to their real context windows (e.g. `kimi-k2.7-code` → 262144, `kimi-k3` → 1048576), and the Moonshot context fallback no longer fires unless the configured model is actually a Kimi model.

### Added

- **Form/JSON Toggle for the Claude Provider Config Editor** (previously unreleased): the Claude provider form can flip between the structured form view and raw JSON editing.

### Notes

- Upstream `v3.19.2` is fully merged in; see the `[3.19.2]` section below for its release notes (Codex usage recount, security hardening, OMO unified config, panel search + bulk toggles, Auth Center usage, batched backup/import).
- If you ran a build that carried the Codex auth bug, stored third-party providers keep their stored config untouched — the live config is normalized at switch time, so no manual cleanup of `~/.codex/config.toml` is needed beyond one provider switch.
- If a Claude Science session still shows its model as unavailable after upgrading, restart Claude Science (or wait for the daemon's model-list cache to expire) so it refetches `/v1/models`, then reselect the model once.
...
**Stats**: 7 commits | 12 files changed | +642 insertions | -90 deletions

### Added

- **Claude Science `/v1/models` Served from Local Proxy**: Claude Science reads `ANTHROPIC_BASE_URL` and calls `/v1/models` through cc-switch's proxy. The route was missing (404), so the Science model dropdown showed "Couldn't load the full model list" and the Fable tier never appeared when configured. `GET /claude-science/v1/models` is now registered alongside the existing messages route and answers with `model_list_response()` for the current claude-science provider, so the model list is served entirely from cc-switch config — no Anthropic OAuth involved.
- **Kimi for Coding Presets Pinned to Their Real Context Windows**: Claude Code caps unknown non-Claude model ids at 200K, so Kimi's Anthropic-protocol presets showed the wrong context unless each preset declared `CLAUDE_CODE_MAX_CONTEXT_TOKENS` / `CLAUDE_CODE_AUTO_COMPACT_WINDOW` explicitly. Every Kimi Claude Code preset in `src/config/claudeProviderPresets.ts` now declares its actual window so the status bar and compact window match reality: `kimi-k2.7-code` → 262144 (was missing entirely), `kimi-k3` → 1048576 (1M, new preset), `kimi-for-coding` → 262144 (already pinned), `k3` → 1048576 (new preset on the Kimi Code service), `k3-256k` → 262144 (new preset on Kimi Code), `kimi-for-coding-highspeed` → 262144 (new preset on Kimi Code, ~5–6× faster output at ~3× token cost). The live-settings helper `is_kimi_for_coding_provider()` now matches both `api.kimi.com/coding` and `api.moonshot.cn/anthropic` so the six presets all pick up the default. Model IDs and context windows sourced from `https://www.kimi.com/code/docs/kimi-code/models`.

### Fixed

- **Claude Desktop Aggregate Providers Could Not Be Added**: `AggregateProviderForm` for Claude Desktop submits a placeholder `settings_config` (an empty object) and no `meta.claudeDesktopMode`. The backend's `provider_mode()` defaulted to `Direct` and ran `validate_direct_provider`, which requires `settings_config.env.ANTHROPIC_BASE_URL` and `ANTHROPIC_AUTH_TOKEN`. The result was a hard rejection with "missing env configuration" the moment a user tried to add a Claude Desktop aggregate provider. `provider_mode()` now returns `Proxy` for aggregate providers (aggregates only make sense through the local router), `validate_provider()` short-circuits for aggregates the same way it does for the official provider (the target-provider legality is already enforced by `aggregate_routes` reference checks), and `apply_provider_to_paths_inner()` no longer demands `claudeDesktopModelRoutes` from an aggregate provider — it just stamps `deployment_mode = "3p"` and writes meta. Two new unit tests in `src-tauri/src/claude_desktop_config.rs` (`claude_desktop_aggregate_provider_passes_validate`, `claude_desktop_aggregate_apply_does_not_require_model_routes`) cover all three paths.
- **Strict Anthropic-Compatible Gateways Rejected Orphan `tool_use`/`tool_result` Pairs**: DeepSeek's official `/anthropic` endpoint, Volcengine Ark Coding Plan, Kimi for Coding and MiniMax all validate that every `tool_use` block in an assistant message has a matching `tool_result` immediately after. When a turn is interrupted after the `tool_use` is persisted, or with DeepSeek V4's parallel tool calls whose results don't all arrive, the gateway rejects with `"tool_use ids were found without tool_result blocks immediately after: call_00_..."`. The local proxy now repairs both directions for non-official endpoints: an orphan `tool_use` gets a synthetic error `tool_result` injected into the following user message (creating one when the turn ends at the assistant message); an orphan `tool_result` is downgraded to a text block. Matching is count-based, not set-based, so parallel/concurrent `tool_use`s that share an id — or whose results partially arrived — each get one synthetic result. Properly paired history and traffic to `api.anthropic.com` pass through untouched. (`fix(proxy): repair orphan tool_use/tool_result pairing for non-official endpoints`, `fix(proxy): count-based orphan tool_use repair for DeepSeek parallel calls`.)
- **GitHub Copilot Stopped Working With Modern Claude Code (2.1.220)**: Claude Code does not format-validate `ANTHROPIC_API_KEY` against `sk-ant-*`; in headless mode the placeholder is sent upstream as-is. The actual failure mode was the interactive custom-API-key approval prompt — defaulting to "No (recommended)" ignores the key and lands the user in "Not logged in". The Copilot key-resolution path is updated to match Claude Code's actual expectation. (upstream #5832.)
- **Hermes Used the Wrong Prompt Filename**: Hermes reads `SOUL.md`, not `AGENTS.md`. The Hermes integration pointed at `AGENTS.md`, so prompts never landed on the persona file Hermes actually loads. The path now resolves to `SOUL.md`, with a regression test asserting the same. (upstream #5779.)

### Notes

- This release sits directly on top of `v3.19.1+ciao.3`. No upstream hand-off this round — every change is in the ciao-only commits merged through PR #13.

---

## [3.19.1+ciao.3] - 2026-08-02

ciao fork release on top of upstream `v3.19.1`. The headline work is a single / aggregate / universal provider-type picker that was extracted from `ProviderForm` into a standalone `AggregateProviderForm` in ciao.3 and then widened beyond Claude and Codex to also cover **Claude Desktop** and **Claude Science** (which share Claude's four-tier Haiku/Sonnet/Opus/Fable route table). Alongside it, Claude Science gains WSL support on Windows: bind a distro explicitly via a UNC config dir, or leave it empty and let CC Switch auto-detect the default distro, the `claude-science` binary and the distro user home. The full upstream `v3.19.1` maintenance pass — DeepSeek/Volcengine Ark Coding Plan moved onto native Codex Responses, the mirrored official DeepSeek catalog, the Tencent Hunyuan (TokenHub) preset, the double-counted Claude Desktop usage fix, the stale Codex `auth.json` cleanup, the `grok update` PATH repair and the Grok Build proxy takeover 404 fix — is merged in. Two web-search provider options that already shipped in `ciao.2` builds are documented below for the first time.
...
**Stats**: 33 commits | 212 files changed | +6,866 insertions | -4,584 deletions

### Added

- **Provider Type Tabs (Single / Aggregate / Universal)**: the "Add Provider" dialog now offers three sibling tabs — single (the classic form), aggregate (Claude-family and Codex only upstream, extended on the ciao side to Claude Desktop and Claude Science) and universal. Aggregate mode is extracted from `ProviderForm` into a standalone `AggregateProviderForm` (name + notes + a tier-route table; tab selection is the enable switch), and editing an aggregate provider opens that dedicated form with live-config loading skipped. `ProviderForm` sheds all aggregate logic (~170 lines of guards and state) and its submit payload is unchanged. Universal providers continue to flow through the existing blacklist path. Localized in all four locales (zh/en/ja/zh-TW).
- **Aggregate Routing Extended to Claude Desktop and Claude Science**: on top of the upstream Claude + Codex coverage, the aggregate tab (and the standalone `AggregateProviderForm`) now accepts `claude-desktop` and `claude-science`, both of which share Claude's four-tier route table. The backend validation (`validate_aggregate_routes`) and referenced-target delete guard accept ClaudeDesktop alongside Claude and Codex, and proxy request routing expands aggregate providers for Claude Desktop traffic through the same `ClaudeTier` classification as Claude. Claude Science aggregate routes were already validated and expanded by the backend; the ciao-side tab is what makes them creatable from the UI.
- **Claude Science Launches Inside WSL on Windows**: point the Claude Science config dir at a UNC path (`\\wsl$\<distro>\...`) to bind a distro explicitly, or leave it empty and let CC Switch auto-detect the default distro, the `claude-science` binary and the distro user home. In WSL2 the proxy URL host is rewritten to the Windows host IP visible from WSL (the default gateway inside the WSL namespace, since Windows loopback is unreachable there); WSL1 keeps the shared loopback. Complex shell scripts are passed through base64 so the Windows → WSL command line cannot corrupt quoting, `wsl.exe` output is decoded as UTF-16LE via BOM or a NUL-ratio heuristic, a double slash in `ANTHROPIC_BASE_URL` is fixed, and the Claude Science launch button no longer requires the local-proxy toggle to be on.
- **Upstream `v3.19.1` Features (Merged In)**: DeepSeek and Volcengine Ark Coding Plan connect to Codex directly over native Responses (no local routing takeover needed), with a general mechanism for mirroring a vendor's own model catalog verbatim — DeepSeek first, including the freeform `apply_patch` registration and GPT-5 harness (`src-tauri/src/resources/codex_deepseek_catalog_template.json`); a Tencent Hunyuan (TokenHub) Codex preset joins the picker under Opensource Official with a 256K context window and `disable_response_storage = true`; built-in pricing lands for eight models that silently billed at zero (`gpt-5.3-codex-spark`, `gemini-3.5-flash-lite`, `kimi-k2.7-code-highspeed`, `glm-5-turbo`, `glm-5v-turbo`, `qwen3.6-flash`, `claude-opus-4-6`, `claude-sonnet-4-6`); Grok Build joins the failover tabs alongside Claude Code, Codex and Gemini and the startup environment-conflict banner detects shell-exported `XAI_API_KEY` or `GROK_DEFAULT_MODEL`; and 3,166 lines of superseded code plus four npm dependencies are removed. See `[3.19.1]` below for the full upstream notes.

### Fixed

- **Claude Desktop Usage Was Counted Twice** (from upstream `v3.19.1`): since v3.18.0 Claude Desktop traffic through the local proxy landed once as a proxy row and once as an imported transcript row — proxy dedup ids were scoped as `session:{app_type}:{provider_id}:{message_id}` for every app except `claude`, which put `claude-desktop` in its own namespace while the transcript importer kept writing the same Claude message under the bare `session:{message_id}` shape with `app_type = 'claude'`. The two apps now share the bare namespace again with a one-way absorption rule. (#5938, #5951)
- **Switching Back to the Official Codex Provider Stranded You on a 401 With No Login Screen** (from upstream `v3.19.1`): switching to a built-in official Codex provider took the config-only write path while a third-party `OPENAI_API_KEY` stayed in `~/.codex/auth.json`, so Codex authenticated to the official endpoint with the wrong credential and never fell through to its login screen. After a successful switch to an official Codex provider, CC Switch now deletes an `auth.json` that contains only `OPENAI_API_KEY` with no first-class credential beside it; OAuth tokens, personal access tokens, agent identities and Bedrock keys all mark the file as real and leave it untouched.
- **`grok update` From Settings Failed With a Bare `os error 2`** (from upstream `v3.19.1`): a GUI-launched app cannot see node, so the lifecycle script's `npm` spawn returned ENOENT and grok surfaced it as a bare error. Lifecycle commands on macOS and Linux now run with the login shell's real PATH merged ahead of the inherited one, and native Grok's update additionally chains the official xAI installer as a fallback.
- **Grok Build Proxy Takeover Returned 404, and Every Request Looked Like a New Session** (from upstream `v3.19.1`): takeover now also pins the backend to Responses, and Grok's own conversation/session headers are read under a distinct `grokbuild_` prefix so per-session grouping in the usage dashboard survives. (#5677)
- **Nine Interface Strings Rendered as Simplified Chinese in Every Language; Traditional Chinese Tool Manager Fell Back to English** (from upstream `v3.19.1`): all nine strings and the thirty tool-manager keys now exist in zh/en/ja/zh-TW, with a locale test asserting matching interpolation variables across all four languages. (#5960, #5943)
- **Codex Universal Children Referenced by Aggregate Routes Could Be Deleted Out From Under You**: `delete_universal` / `sync_universal_to_apps` now block removing the generated `universal-codex-{id}` child when a codex aggregate route references it, mirroring the existing Claude child guard. `getAggregateRouteTargets` also takes `appId` and keeps the built-in Codex official provider selectable, matching the backend takeover policy. Addresses Codex review feedback on #5937.
- **Claude Science WSL Build Failed With `unused_variable` Outside Windows**: the non-Windows branch of the Claude Science WSL launch path referenced the distro binding in a way rustc flagged as unused with `-D warnings` on macOS/Linux CI; the binding is now used (or elided) on every platform. (98a8288, 31dfe03)

### Changed

- **CI Runs on `ciao` / `ciao-dev` Branches as Well as `main`**: the upstream-inherited CI trigger was `main`-only, so the fork's PRs never fired anything except the labeler. `.github/workflows/ci.yml` now triggers on `ciao` and `ciao-dev` branches too, alongside the existing `main` runs. A round of rustfmt / prettier / clippy cleanups across proxy and provider modules lands in the same window to satisfy the new matrix.
- **Home-Mutating Provider Tests Now Run Under `#[serial]`**: 17 provider tests rewrite the process-wide `HOME` / `CC_SWITCH_TEST_HOME` through `with_test_home` / `TempHome`, and on parallel runners they raced the existing serial gemini-scrub and pricing tests — a mid-test env swap or temp-dir teardown made reads land in a deleted or foreign home. They now run under `#[serial]`, and the upstream self-heal path that required it ships untouched.

### Added (shipped in ciao.2, documented here)

- **Claude Provider Empty Search-Result Filter (Proxy Takeover)**: Claude providers gain a per-provider "Filter empty search results" checkbox (stored as `meta.webSearchResultFilter`, default off). Some third-party relay channels fake WebSearch support by injecting a `"Search results for query:"` text block into the assistant content of Anthropic Messages responses; when the block carries no real results after the colon, enabling the filter makes the local proxy (127.0.0.1:15721) drop the whole block — in streaming responses the text block is buffered until `content_block_stop` for the keep/drop decision (kept blocks flush as start + one merged delta + stop, and forwarded `content_block_*` indices are renumbered to stay gap-free), in non-streaming responses the parsed body's `content[]` is stripped before forwarding, and on the request side already-persisted empty headers are removed from message history so the model stops parroting them. Blocks with real content after the colon and all non-matching blocks pass through byte-identical; a placeholder text block keeps any content array non-empty when every block is dropped. Localized in all four locales (zh/en/ja/zh-TW).
- **Claude Provider Web Search Compatibility Layer**: Claude providers gain a per-provider "Web Search" policy (auto / disabled / enabled, stored as `meta.webSearchCompat` in the provider form). Some third-party relay channels (new-api / one-api style gateways) do not support — or only pretend to support — Claude Code's WebSearch tool and inject results into the conversation as `"Search results for query:"` plain text; setting the policy to "disabled" makes CC Switch inject `permissions.deny: ["WebSearch"]` when writing the live `~/.claude/settings.json`, so Claude Code disables the tool locally for that provider while supported providers stay untouched. The injected rule is an ownership sentinel: it lives only in the live file, is stripped symmetrically on switch-away backfill and common-config snippet extraction (a user-written `WebSearch` deny in the provider's own stored config is never removed), and "auto" falls back to a built-in host/model blacklist that starts empty by design — a missed detection only means the user flips the toggle manually, while a false positive would strip working search. Localized in all four locales (zh/en/ja/zh-TW).

### Internal

- **Dropped Unity2.ai as a Sponsor**: the Unity2.ai preset, README row and i18n strings are removed across all four locales; existing providers keep their stored base URL.

### Upgrade notes

- No database schema migration in this release — `SCHEMA_VERSION` stays at 16.
- Aggregate providers you already saved keep their stored config; the ciao.3 change widens which apps the aggregate tab is creatable from, it does not migrate existing single-mode providers into aggregate mode.
- Claude Desktop aggregate routes share Claude's four-tier (Haiku/Sonnet/Opus/Fable) mapping; selecting Fable requires the upstream to actually expose a Fable-tier model, otherwise the tier falls through to the default. Claude Science aggregate routes work the same way.
- On Windows the Claude Science launch button no longer requires the local-proxy toggle; it now launches inside WSL when a config dir is set to a UNC path or auto-detected. Removing the WSL config dir returns to launching on the Windows host directly.
- See the `[3.19.1]` upgrade notes for the upstream-v3.19.1 retroactive items: double-counted Claude Desktop usage corrects itself within the detail-row retention window; eight newly priced models backfill at startup; four repriced models only re-price zero-cost rows; DeepSeek/Volcengine/Tencent preset changes apply only to newly created providers; `deepseek-v4-pro` is not usable over the direct connection yet — use `deepseek-v4-flash`, or switch the provider's API format back to OpenAI Chat and re-enable local routing takeover; mirrored DeepSeek catalog declares a minimum Codex client of 0.144.0; direct (non-takeover) Codex traffic no longer shows up under its provider name in the usage dashboard; Grok Build takeover now rewrites backend to Responses (live, not stored); macOS/Linux tool lifecycle commands now run with the login shell's PATH; environment-conflict detection moved to prefix matching for Claude Code/Codex/Gemini; deep-link import confirmations reveal four leading characters instead of eight.
...
## [3.19.0+ciao.2] - 2026-07-31

ciao fork release on top of upstream v3.19.0: aggregate provider routing generalized from Claude to Codex, aggregate-card UX improvements, and a database rework that keeps the schema byte-compatible with upstream (v16) so official and ciao builds can share one database in both directions without "database version too new" errors.

### Added

- **Aggregate Providers for Codex (Exact-Model Routing)**: Aggregate providers — providers with no endpoint or credentials of their own that route each request to another provider based on the model — now work in Codex in addition to Claude / Claude Science. `meta.aggregateRoutes` gains a free-key `custom` route table matched by exact request model name (e.g. `gpt-5.5 → StepFun(step-3.5-flash)`), while the Anthropic-protocol apps keep the fixed Haiku/Sonnet/Opus/Fable tiers. Proxy expansion is generalized over both key kinds; a routed Codex target inherits its endpoint/credentials untouched and only its upstream model is rewritten, so any regular provider — including Chat- or Anthropic-protocol ones — can serve as a Codex route target. During proxy takeover the Codex live config seeds a valid `model_providers` section and synthesizes `modelCatalog` from the route table, so the Codex client's model picker offers only routed models. Route validation, target delete protection, and the switch / disable-takeover guards apply uniformly across all three apps.
- **Per-App Aggregate Route Editor**: The provider form renders the aggregate section for Codex too — a dynamic row editor (request model + target provider + upstream model) next to Claude's four-tier table. The request-model field offers built-in OpenAI model suggestions, and the upstream-model field can fetch the target provider's live `/v1/models` list (merged with its configured catalog models), so neither value has to be typed blind. The website URL field is hidden while aggregate mode is enabled, and the section hint now describes per-model routing for Codex instead of the Claude tiers. All strings localized in zh / zh-TW / en / ja.
- **Aggregate Card Target Tags and Usage Refresh**: Aggregate provider cards show the routed target providers as tags under the provider name (tooltip lists each tier/model mapping) instead of the meaningless website-URL line, and gain a "refresh usage" button that refetches usage for every routed target provider at once.

### Fixed

- **Editing an Active Aggregate Provider No Longer Errors**: Updating the current provider that already is aggregate — including turning aggregation off — no longer fails with "the active provider can only be converted to an aggregate provider while proxy takeover is enabled"; the guard now only blocks an actual non-aggregate → aggregate conversion.
- **Codex Aggregate Providers Can Be Saved**: The Codex `auth`-required settings validation no longer rejects an aggregate provider's intentional empty placeholder config.

### Changed

- **Database Schema Stays at v16 (Official-Compatible)**: The Claude Science proxy config no longer requires a schema migration — its takeover/auto-failover flags, cost multiplier, and pricing source are stored in the generic `settings` table instead of a `proxy_config` row, keeping `SCHEMA_VERSION` at 16 and the table structure identical to upstream. A targeted self-heal runs at startup for databases already migrated by earlier ciao builds: when `user_version` is 17 and the `proxy_config` DDL carries the claude-science signature, the row's full state is moved into `settings`, the row is deleted, and the version is set back to 16. Databases carrying any other future version still hit the normal "database version too new" guard, and the startup precheck was fixed so it no longer intercepts this self-heal path. Upgrading or downgrading between official and ciao builds is now seamless in both directions.
