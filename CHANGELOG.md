...
- **Claude Science in WSL (Windows)**: Claude Science can now launch inside a WSL distribution. Point the Claude Science config dir at a UNC path (`\\wsl$\<distro>\...`) to bind a distro explicitly, or leave it empty and cc-switch auto-detects the default distro, the `claude-science` binary and the distro user home. In WSL2 the proxy URL host is rewritten to the Windows host IP visible from WSL (the default gateway in the WSL namespace, since Windows loopback is unreachable there); WSL1 keeps the shared loopback. Complex launch scripts are passed through base64 so the Windows → WSL command line cannot corrupt quoting, `wsl.exe` output is decoded as UTF-16LE via BOM or a NUL-ratio heuristic, a double slash in `ANTHROPIC_BASE_URL` is fixed, and the Claude Science launch button no longer requires the local-proxy toggle.
- **Provider Type Tabs (Single / Aggregate / Universal)**: adding a provider now offers sibling tabs — single (the classic form), aggregate (Claude-family and Codex only), and universal. Aggregate mode is extracted from `ProviderForm` into a standalone `AggregateProviderForm` (name/notes + route table; tab selection is the enable switch), and editing an aggregate provider opens that dedicated form with live-config loading skipped. On the ciao side the aggregate tab and its backend validation / proxy expansion are extended beyond Claude and Codex to **Claude Desktop** and **Claude Science**, which share Claude's four-tier (Haiku/Sonnet/Opus/Fable) route table.
- **Upstream v3.19.1**: DeepSeek and Volcengine Ark Coding Plan connect to Codex directly over native Responses (no local routing takeover needed), with a general mechanism for mirroring a vendor's own model catalog verbatim (DeepSeek first, including the freeform `apply_patch` registration and GPT-5 harness); a Tencent Hunyuan (TokenHub) Codex preset joins the picker; built-in pricing lands for eight models that silently billed at zero; and 3,166 lines of superseded code plus four npm dependencies are removed.

### Fixed (from upstream v3.19.1)

- **Claude Desktop usage was counted twice** since v3.18.0 — once as a proxy row and once as an imported session row (#5938, #5951).
- Switching back to the built-in official Codex provider left a third-party key in `auth.json`, so Codex authenticated to the official endpoint with the wrong credential and returned 401.
- `grok update` failed from Settings with a bare `os error 2` because a GUI-launched app cannot see node.
- Grok Build's proxy takeover returned 404 on any non-Responses backend while every request minted a fresh session id (#5677).
- Nine interface strings that rendered as Simplified Chinese in every language are localized, and the Traditional Chinese About page gains the thirty tool-manager strings it was missing; deep-link import confirmations mask more and truncate less.

### Changed

- **CI hardening**: rustfmt / prettier / clippy cleanups across proxy and provider modules, and 17 home-mutating provider tests now run under `#[serial]` so they no longer race the gemini scrub and pricing tests on parallel runners.
